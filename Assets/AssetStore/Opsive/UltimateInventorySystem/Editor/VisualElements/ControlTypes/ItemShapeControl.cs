...
namespace Opsive.UltimateInventorySystem.Editor.VisualElements.ControlTypes
{
        ...
        public class BooleanGridField : VisualElement
        {
                ...
                for (int y = 0; y < m_ItemShape.Size.y; y++) {
                    ...
                    for (int x = 0; x < m_ItemShape.Size.x; x++) {
                        ...
                        if (x == m_ItemShape.Anchor.x && y == m_ItemShape.Anchor.y) {
                            toggle.AddToClassList(ControlTypeStyles.BooleanGridAnchor);
                        } else {
                            toggle.RemoveFromClassList(ControlTypeStyles.BooleanGridAnchor);
                        }
            ...
            void SetAnchor(DropdownMenuAction action)
            {
                ...
                if (previousAnchor != m_ItemShape.Anchor) {
                    GetToggle(previousAnchor).RemoveFromClassList(ControlTypeStyles.BooleanGridAnchor);
                    GetToggle(m_ItemShape.Anchor).AddToClassList(ControlTypeStyles.BooleanGridAnchor);
                }
    ...
    public class AddRemoveField : VisualElement
    {
        ...
        {
            AddToClassList(ControlTypeStyles.AddRemoveField);

            m_Label = new Label(text);
            Add(m_Label);

            m_Remove = new Button();
            m_Remove.AddToClassList(ControlTypeStyles.AddRemoveFieldButton);
            m_Remove.text = "-";
            m_Remove.clicked += () => OnAddRemove?.Invoke(false);
            Add(m_Remove);

            m_Add = new Button();
            m_Add.AddToClassList(ControlTypeStyles.AddRemoveFieldButton);
            m_Add.text = "+";
