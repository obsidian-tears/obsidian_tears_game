...
namespace Opsive.UltimateInventorySystem.Editor.Managers
{
    ...
    public class DatabaseDuplicator
    {
        public static InventorySystemDatabase DuplicateDatabase(InventorySystemDatabase oldDatabase,
            string newDatabasePath, bool keepReferencesToPrefabs = true)
        {
            var databaseDuplicator = new DatabaseDuplicator();
            return databaseDuplicator.Duplicate(oldDatabase, newDatabasePath, keepReferencesToPrefabs);
        }
        ...
        private Dictionary<Object, Object> m_AttributePrefabDictionary;

        private InventorySystemDatabase m_NewDatabase;
        private HashSet<object> m_HashSet;
        protected bool m_KeepReferencesToPrefabs;
        protected string m_NewDatabasePath;
        protected string m_NewDatabaseFolderPath;
        ...
        /// <param name="keepReferencesToPrefabs">Keep the prefab references as is, or duplicate prefabs?</param>
        /// <returns>The new duplicated database.</returns>
        private InventorySystemDatabase Duplicate(InventorySystemDatabase oldDatabase, string newDatabasePath, bool keepReferencesToPrefabs)
        {
            m_KeepReferencesToPrefabs = keepReferencesToPrefabs;
            m_AttributePrefabDictionary = new Dictionary<Object, Object>();
            ...
            m_NewDatabaseFolderPath = newFolderPath;

            Debug.Log($"Duplicating the '{oldDatabase.name}' database to {newDatabasePath}.");
        ...
        private void FixAttributeReferences(AttributeBase attribute)
        {
            ...
            // Scan non-Object types and replace InventoryObject references.
            ...
            // It could be a Prefab
            if(m_KeepReferencesToPrefabs){ return; }

            if(type == typeof(Component) 
               || type.IsSubclassOf(typeof(Component))
               || type == typeof(GameObject) 
               || type.IsSubclassOf(typeof(GameObject))) {
                var duplicateObject = DuplicatePrefabObject(value as Object);
                if (duplicateObject != null) {
                    attribute.SetOverrideValueAsObjectWithoutNotify(duplicateObject);
                }
            }
        }
        ...
        private void SwapObjectByNewOne(ref object obj, Type type, FieldInfo field)
        {
            ...
            if(type == typeof(Component) 
               ...
               || type == typeof(GameObject) 
               || type.IsSubclassOf(typeof(GameObject))) {
                var duplicateObject = DuplicatePrefabObject(obj as Object);
                if (duplicateObject != null) {
                    field.SetValue(obj, duplicateObject);
                }
            }
        }
        ...
        private Object DuplicatePrefabObject(Object original)
        {
            if (original == null) {
                return null;
            }
            
            if (m_AttributePrefabDictionary.TryGetValue(original, out var duplicate)) {
                return duplicate;
            }

            var duplicateAsset = DuplicateAssetToPath(original, m_NewDatabaseFolderPath+"/Prefabs/");

            if (duplicateAsset == null) {
                return null;
            }

            if (duplicateAsset is Component duplicateComponent && original is Component originalComponent) {
                m_AttributePrefabDictionary[originalComponent] = duplicateComponent;
                m_AttributePrefabDictionary[originalComponent.gameObject] = duplicateComponent.gameObject;
            }
            
            if (duplicateAsset is GameObject duplicateGameObject && original is GameObject originalGameObject) {
                m_AttributePrefabDictionary[duplicateGameObject] = originalGameObject;
            }

            return duplicateAsset;
        }
        
        private static Object DuplicateAssetToPath(Object sourceObj, string folderPath)
        {
            var previousPath = AssetDatabase.GetAssetPath(sourceObj);

            var suffix = Path.GetExtension(previousPath);

            var newPath = folderPath + sourceObj.name + suffix;
            newPath = AssetDatabaseUtility.ValidatePath(newPath);

            if (newPath == null) {
                Debug.LogWarning($"The {sourceObj.name} in the database failed to duplicate.");
                return null;
            }
            ...
            var duplicateObj = (Object)AssetDatabase.LoadAssetAtPath<Object>(newPath);
