namespace Opsive.UltimateInventorySystem.Editor.Managers
{
    ...
    public class InventorySystemDatabaseMainField : VisualElement
    {
        ...
        private void DuplicateDatabase()
        {
            ...
            var keepReferencesToPrefabs = EditorUtility.DisplayDialog("Duplicate attribute prefab References?",
                "Some Attributes may be referencing prefabs used in a demo scene. Do you wish to keep those references, or duplicate all prefabs?",
                "Keep references to prefabs", "Duplicate all referenced Prefabs");
            ...
            if (path.Length != 0 && Application.dataPath.Length < path.Length) {
                ...
                var database = DatabaseDuplicator.DuplicateDatabase(m_DatabaseField.value as InventorySystemDatabase, path, keepReferencesToPrefabs);
