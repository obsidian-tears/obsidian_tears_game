...
namespace Opsive.UltimateInventorySystem.ItemActions
{
    ...
    public class AssignHotbarItemAction : ItemActionWithAsyncFuncActionPanel<int>
    {
        ...
        [Tooltip("If the Action Buttons are ItemViewSlots then assign the ItemView Slots with the hotbar itemInfos.")]
        [SerializeField] protected bool m_UseItemViewSlotIfAvailable = true;
        ...
        protected override void InvokeActionInternal(ItemInfo itemInfo, ItemUser itemUser)
        {
            ...
            //Call the base function to open the panel.
            base.InvokeActionInternal(itemInfo, itemUser);


            if (m_UseItemViewSlotIfAvailable) {
                //After opening the panel draw the item view slots if available.
                var actionButtons = m_AsyncFuncActionPanel.ItemActionButtons;
                for (int i = 0; i < actionButtons.Count; i++) {
                    if (actionButtons[i] is ItemViewSlot itemViewSlot) {
                        itemViewSlot.SetItemInfo(m_ItemHotbar.GetItemAt(i));
                    }
                }
            }
        }
